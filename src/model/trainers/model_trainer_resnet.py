...
class ModelTrainerResnet(ModelTrainer):
    _model: Model
    ...
    def train_model(self):
        ...
        logging.info(f"N:\t{N}")
        logging.info(f"Train:\t{len(train_df)}, {len(train_df) / N * 100:.0f}%")
        logging.info(f"Val:\t{len(val_df)}, {len(val_df) / N * 100:.0f}%")
        logging.info(f"Test:\t{len(test_df)}, {len(test_df) / N * 100:.0f}%")

        train_ds = _df_to_dataset(train_df, batch_size=32, repeat=True)
        val_ds = _df_to_dataset(val_df, batch_size=8)
        test_ds = _df_to_dataset(test_df, batch_size=8)
    ...
    def update_saved_model(self, target_path: Union[PathLike, str]):
        with TemporaryDirectory() as tempdir:
            self._model.save(tempdir)
            # atomic replacement
            shutil.move(tempdir, target_path)
...
def _df_to_dataset(df, shuffle=True, batch_size=None, repeat=False):
    ds = tf.data.Dataset.from_generator(_get_dataset_generator(df), output_types=(tf.float32, tf.float32),
